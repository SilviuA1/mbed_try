...
void ADC_Config_Alt(ADC_Type *, tADC_ConfigPtr);

void ADC_Read_Cal(ADC_Type *, tADC_Cal_Blk *);
...
******************************************************************************/
unsigned char ADC_Cal(ADC_Type *adcmap)
{

  unsigned short cal_var;
  
  adcmap->SC2 &=  ~ADC_SC2_ADTRG_MASK ; // Enable Software Conversion Trigger for Calibration Process    - ADC0_SC2 = ADC0_SC2 | ADC_SC2_ADTRGW(0);
  adcmap->SC3 &= ( ~ADC_SC3_ADCO_MASK & ~ADC_SC3_AVGS_MASK ); // set single conversion, clear avgs bitfield for next writing
  adcmap->SC3 |= ( ADC_SC3_AVGE_MASK | ADC_SC3_AVGS(AVGS_32) );  // Turn averaging ON and set at max value ( 32 )
  
  
  adcmap->SC3 |= ADC_SC3_CAL_MASK ;      // Start CAL
  while ( (adcmap->SC1[A] & ADC_SC1_COCO_MASK ) == COCO_NOT ); // Wait calibration end
  	
  if ((adcmap->SC3 & ADC_SC3_CALF_MASK) == CALF_FAIL )
  {  
  ...
  adcmap->MG = ADC_MG_MG(cal_var);
  
  adcmap->SC3 &= ~ADC_SC3_CAL_MASK ; /* Clear CAL bit */
...
void ADC_Config_Alt(ADC_Type *adcmap, tADC_ConfigPtr ADC_CfgPtr)
{
	adcmap->CFG1 = ADC_CfgPtr->CONFIG1;
	adcmap->CFG2 = ADC_CfgPtr->CONFIG2;
	adcmap->CV1  = ADC_CfgPtr->COMPARE1;
	adcmap->CV2  = ADC_CfgPtr->COMPARE2;
	adcmap->SC2  = ADC_CfgPtr->STATUS2;
	adcmap->SC3  = ADC_CfgPtr->STATUS3;
 //ADC_PGA_REG(adcmap)  = ADC_CfgPtr->PGA;
	adcmap->SC1[A]= ADC_CfgPtr->STATUS1A;
	adcmap->SC1[B]= ADC_CfgPtr->STATUS1B;
}


void ADC_Read_Cal(ADC_Type *adcmap, tADC_Cal_Blk *blk)
{
  blk->OFS  = adcmap->OFS;
  blk->PG   = adcmap->PG;
  blk->MG   = adcmap->MG;
  blk->CLPD = adcmap->CLPD;
  blk->CLPS = adcmap->CLPS;
  blk->CLP4 = adcmap->CLP4;
  blk->CLP3 = adcmap->CLP3;
  blk->CLP2 = adcmap->CLP2;
  blk->CLP1 = adcmap->CLP1;
  blk->CLP0 = adcmap->CLP0;
  blk->CLMD = adcmap->CLMD;
  blk->CLMS = adcmap->CLMS;
  blk->CLM4 = adcmap->CLM4;
  blk->CLM3 = adcmap->CLM3;
  blk->CLM2 = adcmap->CLM2;
  blk->CLM1 = adcmap->CLM1;
  blk->CLM0 = adcmap->CLM0;
...
void InitADC0()
{
    ...
    ADC_Config_Alt(ADC0, &Master_Adc0_Config);  // config ADC
 
    // Calibrate the ADC in the configuration in which it will be used:
     ADC_Cal(ADC0);                    // do the calibration
     ...
     ADC_Config_Alt(ADC0, &Master_Adc0_Config);
}
...
void TFC_InitADCs()
{
	...
	PIT->MCR |= PIT_MCR_FRZ_MASK; // stop the pit when in debug mode
...
void PIT_IRQHandler()
{
	PIT->CHANNEL[0].TFLG = PIT_TFLG_TIF_MASK; //Turn off the Pit 0 Irq flag
	...
	ADC0->SC1[A]  =  TFC_POT_0_ADC_CHANNEL | ADC_SC1_AIEN_MASK;  //Start the State machine at POT0
}
...
void ADC0_IRQHandler()
{
	...
	switch(CurrentADC_State)
	{
		...
		case ADC_STATE_CAPTURE_POT_0:
				...
				PotADC_Value[0] = ADC0->R[A];
				ADC0->CFG2  &= ~ADC_CFG2_MUXSEL_MASK; //Select the A side of the mux
				ADC0->SC1[A]  =  TFC_POT_1_ADC_CHANNEL | ADC_SC1_AIEN_MASK;
		...
		case ADC_STATE_CAPTURE_POT_1:
				...
				PotADC_Value[1] = ADC0->R[A];
				ADC0->CFG2  |= ADC_CFG2_MUXSEL_MASK; //Select the B side of the mux
				ADC0->SC1[A]  =  TFC_BAT_SENSE_CHANNEL| ADC_SC1_AIEN_MASK;
		...
		case ADC_STATE_CAPTURE_BATTERY_LEVEL:
				...
				ADC0->CFG2  |= ADC_CFG2_MUXSEL_MASK; //Select the B side of the mux
				ADC0->SC1[A]  =  TFC_LINESCAN0_ADC_CHANNEL | ADC_SC1_AIEN_MASK;
		...
		case ADC_STATE_CAPTURE_LINE_SCAN:
					...
					if(CurrentLineScanPixel<128)
					{
						...
						{
							LineScanImage0WorkingBuffer[CurrentLineScanPixel] = ADC0->R[A];
							ADC0->SC1[A]  =  TFC_LINESCAN1_ADC_CHANNEL | ADC_SC1_AIEN_MASK;
							CurrentLineScanChannel = 1;
							
						}
						else
						{
							LineScanImage1WorkingBuffer[CurrentLineScanPixel] = ADC0->R[A];
							ADC0->SC1[A]  =  TFC_LINESCAN0_ADC_CHANNEL | ADC_SC1_AIEN_MASK;
							CurrentLineScanChannel = 0;
